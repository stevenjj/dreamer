...
int32_t posActl[NUM_DOFS] = {ENC_CNTR};
int32_t posCtrl[NUM_DOFS] = {ENC_CNTR};
int32_t posDesr[NUM_DOFS] = {
...
uint32_t median2(uint32_t, uint32_t, uint32_t);
void mjcCtrlr(void);
...
const uint32_t vel_length = 23;
 
void calcVel(void) {
    static int32_t posPast[NUM_DOFS][vel_length] = {0};
    ...
    for(uint32_t i = 0; i < NUM_DOFS; i++) {
        ...
        start = median2(posPast[i][(next + 1)%vel_length], posPast[i][(next + 2)%vel_length], posPast[i][(next + 3)%vel_length]);
        stop = median2(posPast[i][next], posPast[i][(next + vel_length - 1)%vel_length], posPast[i][(next + vel_length - 2)%vel_length]);
        velActl[i] = (stop - start);
        
        // High pass filter, most noise is (-1, 1)
        if(velActl[i]*velActl[i] == 1)
            velActl[i] = 0;
    }

    next = (next + 1)%vel_length;
}
...
const uint32_t acc_length = 23;
 
void calcAcc(void) {
    static int32_t velPast[NUM_DOFS][acc_length] = {0};
    static uint32_t next = 0;
    uint32_t start = 0;
    uint32_t stop = 0;
    
    for(uint32_t i = 0; i < NUM_DOFS; i++) {
        velPast[i][next] = velActl[i];
        start = median2(velPast[i][(next + 1)%acc_length], velPast[i][(next + 2)%acc_length], velPast[i][(next + 3)%acc_length]);
        stop = median2(velPast[i][next], velPast[i][(next + acc_length - 1)%acc_length], velPast[i][(next + acc_length - 2)%acc_length]);
        accActl[i] = (stop - start);
        
        // High pass filter, most noise is (-1, 1)
        if(accActl[i]*accActl[i] == 1)
            accActl[i] = 0;
    }

    next = (next + 1)%acc_length;
}
...
uint32_t median2(uint32_t num1, uint32_t num2, uint32_t num3) {
    uint32_t sort[3] = {0};
    uint32_t temp = 0;
...
// Single joint we will us for mjc analysis
uint32_t jSpecial = 11;
...
const uint32_t lookPeriodMin = 1;
const uint32_t lookPeriodMax = 6;
const int32_t movePeriod_n = 3;
const int32_t movePeriod_d = 4;
...
int32_t posOld[NUM_DOFS] = {
    J00_Cnt,  J01_Cnt,  J02_Cnt,  J03_Cnt,  J04_Cnt,  J05_Cnt,
    J06_Cnt,  J07_Cnt,  J08_Cnt,  J09_Cnt,  J10_Cnt,  J11_Cnt};
int32_t posNew[NUM_DOFS] = {
    J00_Cnt,  J01_Cnt,  J02_Cnt,  J03_Cnt,  J04_Cnt,  J05_Cnt,
    J06_Cnt,  J07_Cnt,  J08_Cnt,  J09_Cnt,  J10_Cnt,  J11_Cnt};
...
void motionPlan() {
    ...
    if(lookCount == 0) {
        ...
        if(moveCount == moveLength) {
            ...
            posNew[0] = J00_Cnt + gazeFocus;       // neck pitch
            posNew[1] = J01_Cnt + gazeYaw*1/2;     // head rotation
            posNew[2] = J02_Cnt - gazeYaw*1/4;     // head roll
            posNew[3] = J03_Cnt + gazePitch*2/3 + gazeFocus;
            posNew[4] = J04_Cnt - gazePitch*1/3;   // eye pitch
            posNew[5] = J05_Cnt - gazeYaw*1/2;     // left eye yaw
            posNew[6] = J06_Cnt - gazeYaw*1/2;     // right eye yaw
            posNew[7] = eyelidsMax + 4*(gazeFocus - focusMax); // eyelids
            posNew[8] = J08_Cnt + gazePitch*2;     // right ear rotation
            posNew[9] = J09_Cnt + gazeFocus*2;     // right ear extension
            posNew[10] = J10_Cnt - gazePitch*2;    // left ear rotation
            posNew[11] = J11_Cnt + gazeFocus*2;    // left ear extension
        ...
        // if gaze is in progress, interpolate between previous and next
            ...
            int32_t moveScalar = mjcTraj((moveLength - moveCount)*65535/moveLength)/256;
            for(uint32_t i = 0; i < NUM_DOFS; i++)
                posDesr[i] = posOld[i] + (posNew[i]-posOld[i])*moveScalar/255;
 
            mjcCtrlr();
            
            UART_OutUDec(posDesr[jSpecial]);
            UART_OutChar(' ');
            UART_OutUDec(posCtrl[jSpecial]);
            UART_OutChar(' ');
            UART_OutUDec(posActl[jSpecial]);
            UART_OutChar(CR);UART_OutChar(LF);
    ...
    if(blinkCount1 == 0) {
        ...
        if(blinkCount2 == 0) {
            ...
            if(blinkCount2 > blinkLength/2)
                posDesr[7] = eyelidsMin + (posDesr[7] - eyelidsMin)            \
...
const uint32_t maxIntLen = 5;

void mjcCtrlr(void) {
    int32_t x1 = posActl[jSpecial];
    int32_t xd1 = velActl[jSpecial];
    int32_t xdd1 = accActl[jSpecial];
    int32_t xd2 = 0;
    int32_t xdd2 = 0;
    int32_t xddd2 = 0;
    int32_t dTemp = moveCount;
    uint32_t intLen = maxIntLen;
    ...
    for(uint32_t ii = 0; ii < intLen; ii++) {
        xd2 = xd1;
        xdd2 = xdd1;
        xddd2 = posDesr[jSpecial]*60/dTemp/dTemp/dTemp - x1*60/dTemp/dTemp/dTemp - xd1*36/dTemp/dTemp - xdd1*9/dTemp;
        x1 = x1 + xd2;
        xd1 = xd1 + xdd2;
        xdd1 = xdd1 + xddd2;
        dTemp--;
...
void Timer1A_Handler(void) {
    ...
    // Diagnostic Output
//    UART_OutUDec(posActl[7]);
//    UART_OutChar(' ');
//    UART_OutUDec(posActl[11]);
//    UART_OutChar(CR);UART_OutChar(LF);
}
