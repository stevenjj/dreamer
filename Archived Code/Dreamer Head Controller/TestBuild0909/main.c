...
const uint32_t blinkLength_d = 2;
...
void motionPlan() {
    ...
    if(lookCount == 0) {
        ...
        if(moveCount == moveLength) {
            ...
            desrNew[8] = J08_Cnt + gazePitch*5;     // right ear rotation
            desrNew[9] = J09_Cnt + gazeFocus*5;     // right ear extension
            desrNew[10] = J10_Cnt - gazePitch*5;    // left ear rotation
            desrNew[11] = J11_Cnt - gazeFocus*5;    // left ear extension
...
//        lookCount--;
    ...
    if(blinkCount1 == 0) {
        ...
        // otherwise, sinusoidally interpolate
        else {
            desrPos[7] = eyelidsMin + (desrPos[7] - eyelidsMin)
                         *cos8(255*blinkCount2/blinkLength)/255;
        }
        blinkCount2--;
    }
    else
