...
int32_t focusCommand = 0;
int32_t pitchCommand = 0;
int32_t yawCommand = 0;
...
void motionPlan() {
    ...
    if(lookCount == 0) {
       ...
       // Diagnostic Output
        UART_OutUDec(desrPos[4]);
        UART_OutChar(' ');
        UART_OutUDec(desrPos[5]);
...
void parse(uint32_t command) {
//    uint32_t address = command&0xF0000000;
...
//    }
    ...
    // Diagnostic Output
    UART_OutChar(CR);UART_OutChar(LF);
    UART_OutUDec(focusCommand);
    UART_OutChar(' ');
    UART_OutUDec(pitchCommand);
    UART_OutChar(' ');
    UART_OutUDec(yawCommand);
    UART_OutChar(CR);UART_OutChar(LF);
}
